...
class mysql{
	...
	//验证口令
	public function checkKeyword($keyword, $room){
        if(empty($keyword) || empty($room)){
            return false;
        }
        return $this->getOne("select keyword from `sign_teacher` 
        where `room` = '".$this->deStr($room)."' AND `keyword` = '".$this->deStr($keyword)."'");
    }
    //检查学生是否存在
    ...
    public function getClass(){
        return $this->getAll("select distinct class from `sign_class`");//distinct列出不同的值
    }
    //调取班级学生信息
    ...
    //设置教师端信息
    //口令签到
    public function setSignin($room,$lat,$long,$keyword,$class,$status='0'){
        if(empty($room) || empty($lat) || empty($long) || empty($class)){
        //if(empty($room) || empty($class)){
            return false;
        }
        //return $this->query("insert into `sign_teacher` (`room`, `lat`, `long`, `class`, `status`) 
        //values ('".$this->deStr($room)."', '".$this->deStr($lat)."', '".$this->deStr($long)."', '".$this->deStr($class)."', '".$this->deStr($status)."')");
    	return $this->query("insert into `sign_teacher` (`room`, `lat`, `long`, `keyword`, `class`, `status`) 
    	values ('".$this->deStr($room)."', '".$this->deStr($lat)."', '".$this->deStr($long)."', '".$this->deStr($keyword)."', '".$this->deStr($class)."', '".$this->deStr($status)."')");
	}
    //写入学生签到信息
    public function Signin($room,$lat,$long,$name,$number,$status='0',$reason=''){
        if(empty($room) || empty($name) || empty($number)){
            return false;
        }
        return $this->query("insert into `sign_student` (`room`, `lat`, `long`, `name`, `number`, `status`, `reason`) 
        values ('".$this->deStr($room)."', '".$this->deStr($lat)."', '".$this->deStr($long)."', '".$this->deStr($name)."',
         '".$this->deStr($number)."', '".$this->deStr($status)."', '".$this->deStr($reason)."')");
    }
    //写入班级花名册信息
    ...
    public function endSignin($room){
        ...
        return $this->query("update `sign_teacher` set `status`= '1', `keyword` = '-1' where `room` = '".$this->deStr($room)."' AND `status` = '0'");
    }
    //清空学生签到表中该教室签到信息
